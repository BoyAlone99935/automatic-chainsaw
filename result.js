document.addEventListener("DOMContentLoaded", () => {
  const movie = JSON.parse(localStorage.getItem
  ("selectedmovie"))
  console.log(movie)
  const generalcontainer = document.querySelector('.generalcontainer')
   const api_key = "994eaa52";
   const movie_title = JSON.parse(localStorage.getItem("title"))
  ...
  .then(data => {
    ...
    const ratiing = document.createElement('p')
    const releasedate = document.createElement('p')
    const director = document.createElement('p')

    year.textContent = `Year : ${data.Year}`
    boxoffice.textContent = `boxOffice : ${data.BoxOffice}`
    ratiing.textContent = `ImdbRating : ${data.imdbRating}`
    releasedate.textContent = `Released : ${data.Released} `
    director.textContent = `Director : ${data.Director} `

    specialdetails.append(year, boxoffice, ratiing, releasedate, director)
